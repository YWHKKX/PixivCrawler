...
func makeSDRequest(tagName, tagString string, config *ImageConfig, extendLoras ...string) *http.Response {
	...
	if config.IsManualSetup() {
		if len(extendLoras) == 0 {
			args_LatentCouplet = append(args_LatentCouplet, []interface{}{
				true,
				"1:1,1:2,1:2", // divisions
				"0:0,0:0,0:1", // positions
				"0.5,0.8,0.8", // weights
				50,            // end_at_step
			}...)
		}
	...
	for poseName, poseConfig := range config.GetPoseConfigs() {
		...
		setArg := func(imageData string, index int) {
			useMask := config.GetAndLoraConfigs()[extendLoras[0]].useMask
			if index == 0 && useMask {
				return
			}
			...
			if index == 0 {
				...
				weight = 1.3
				ctype = "depth"
				cmode = "My prompt is more important"
			} else if index == 1 {
				guidance_start = 0.0
				guidance_end = 0.7
				weight = 1.2
		...
		if _, err := os.Stat(poseFile); err == nil {
			...
			for index := 0; index < memberNum-2; index++ {
		...
		/*
			coupleFile := strings.Split(poseFile, "_")[0] + "_couple.json"
			coupleData, err := ioutil.ReadFile(coupleFile)
			if err == nil {
				var latentCouplet map[string]interface{}
				utils.Infof("Find Latent Couple config: %s", coupleFile)

				err := json.Unmarshal(coupleData, &latentCouplet)
				if err != nil {
				...
				if len(args_LatentCouplet) > 0 {
					args_LatentCouplet[0]["divisions"] = latentCouplet["divisions"]
					args_LatentCouplet[0]["positions"] = latentCouplet["positions"]
				} else {
					args_LatentCouplet = append(args_LatentCouplet, map[string]interface{}{
						"enabled":     true,
						"divisions":   latentCouplet["divisions"],
						"positions":   latentCouplet["positions"],
						"weights":     "0.5,0.8,0.8",
						"end_at_step": 50,
					})
				}
